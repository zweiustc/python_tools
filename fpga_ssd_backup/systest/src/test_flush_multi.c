...
int main(void)
{
	...
	for (i = 0; i < 150; i++) {
		  ...
		  if (pthread_create(&tidp[i], NULL, thr_fn_flush, NULL) != 0) {
		    		SSD_CYCLE_INFO("flush",i );
                        
				SSD_ERR("03-02","03","cant not creat thread flush");
	...
	for (i = 0; i < 150; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
				...
				SSD_ERR("03-02","03","cant not join thread flush");   
				ret = SSD_TEST_ERROR;
		}
      	}
	...
	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("03-02","ALL","multi thread flush test pass");
	else
		SSD_CASE_INFO("03-02","ALL","multi thread flush test fail");
	return 0;
}
