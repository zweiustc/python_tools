...
int main(void)
{
	...
	SSD_CASE_INFO("24-03","ALL","TEST the multi thread  read delete for one channel");

        SSD_CASE_INFO("24-03","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("24-03","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("24-03","01","OPEN THE SSD DEVICE");
        } 
  
    
	SSD_CASE_INFO("24-03","02","gen randon one channel id");
    	NUM=3;
		
	SSD_THREAD=300;
		
	TIMES=NUM*SSD_THREAD;
    	gen_order_id(TIMES,44);  //gen the id 300*20
	 	for (i=0;i<TIMES;i++){
                id=store_write[i];
	...
	SSD_CASE_INFO("24-02","03","every thread  to read delete one channel");
    	SSD_THREAD=300;
   	struct thread_coeff para_1[300];
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_read, &para_1[i]) != 0) {
			...
			SSD_CYCLE_INFO("read",i );
                        SSD_ERR("24-03","03","cant not creat thread read");
			}
		}else{
			...
			SSD_CYCLE_INFO("delete",i );
                        SSD_ERR("24-03","03","cant not creat thread delelte");
                        }
		}
	}
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
			...
			SSD_CYCLE_INFO("join",i );
                        SSD_ERR("24-03","03","cant not join thread d");		
}
 
	}
    
     	SSD_CASE_INFO("24-03","04","ssd_close");
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("24-03","ALL","TEST the multi thread  read delete one channel test pass");
	else
		SSD_CASE_INFO("24-03","ALL","TEST the multi thread  read delete one channel  test fail");
