...
#define POL (0x11edc6f41LL)

/*__attribute__((always_inline))*/ inline int baidu_crc32_byte(char const *src,int crc, int length)
{
...
inline int crc32c_qw_pipeline(char const *src,int crc, unsigned long long *table)
{
        int crc_out ;
	//rdi,rsi,rdx,rcx,r8,r9 
	//sub_block size is 512B
        __asm__ __volatile__(
		"xor %%r10, %%r10;"
		"xor %%r8, %%r8;"
		"xor %%r9, %%r9;"
		...
		//"crc32q (%%rdi,%%r10), %%rsi;"
		".byte 0xf2, 0x4a, 0x0f, 0x38, 0xf1, 0x34, 0x17;"
	//	"crc32q 1024(%%rdi,%%r10), %%r8;"
		".byte 0xf2, 0x4e, 0x0f, 0x38, 0xf1, 0x84, 0x17;"
		".byte 0x00, 0x04, 0x00, 0x00;"
	//	"crc32q 2048(%%rdi,%%r10), %%r9;"
		".byte 0xf2, 0x4e, 0x0f, 0x38, 0xf1, 0x8c, 0x17;"
		...
		"xor %%rsi, %%rsi;"
		//"crc32q %%r10, %%rsi;"
		".byte 0xf2, 0x49, 0x0f, 0x38, 0xf1, 0xf2;"
		"xor %%rsi, %%r8;"
		//merge block 1 and block2
		"xor %%r10, %%r10;"
		"xor %%rcx, %%rcx;"
		"3:"
		"mov %%r8, %%rdi;"
		...
		"xor %%rsi,%%r9;"
                ...
                "mov %%r9d,%%eax;"
                :"=a"(crc_out),"=D"(src),"=S"(crc),"=d"(table)
                :"D"(src),"S"(crc),"d"(table)
                :"memory","r8","r9","r10","rcx"
                );
        return crc_out;
...
inline int  baidu_crc32_qw( char const *src, int crc,unsigned int length)
{
 ...
 inline int  baidu_crc32_fast( char const *src, int crc,unsigned int length, unsigned long long *table)
{
	int crc_total =crc;
        unsigned i=0,left = length;
		
	while( left >0 ){
	if(left >= 1024*3)
	{
		crc_total = crc32c_qw_pipeline(src +i,crc_total,table);
		i = i+ 1024*3;
		left = length -i;
	...
	{
		crc_total = baidu_crc32_qw(src+i,crc_total,left);
		left = 0;
	}
	}
	return crc_total;
...
static inline int check_sse4_2()
{
        int ret;
        __asm__ __volatile__ (
                "mov $1, %%rax;"
...
inline unsigned long long pclmul(unsigned long long crc, unsigned long long k)
{
...
inline unsigned long long reflect (unsigned long long crc, unsigned long long bitnum) {
 
 		// reflects the lower 'bitnum' bits of 'crc'

 		unsigned long long i, j=1, crcout=0;
 
		for (i=(unsigned long long)1<<(bitnum-1LL); i; i>>=1) {
 			if (crc & i) crcout|=j;
			j<<= 1;
 	}
 	return (crcout);
...
inline unsigned long long mod2_step1(unsigned long long data, unsigned long long poly, unsigned poly_bit, int data_bit)
{
	unsigned long long remainder = 0;
	int i;
	remainder = data;
	unsigned long long pol = poly << (64 - poly_bit);
	int shift = data_bit ;//+1 - (poly_bit -1);
	i=0;
	while(i < shift){
		if(remainder & 0x8000000000000000LL){
			remainder  = (remainder ^ pol);
		}
		else{
...
inline unsigned long long mod2_step2(unsigned long long data)
{
	unsigned long long remainder = data;
...
inline unsigned long long mod2(unsigned long long data, unsigned long long poly, unsigned poly_bit, int data_bit)
{
	unsigned long long remainder = 0;
	int i;
	remainder = data;
...
inline void gen_table(unsigned long long * table, unsigned long long constant)
{

	unsigned long long i=0,j=0;
	unsigned long long bit_hit, entry_t; 
	for(i=0; i< 256; i++){
		entry_t = 0LL;
		for(j=1; j<256; j = j*2){
			bit_hit = i & j;
			entry_t = entry_t ^ (bit_hit * constant);
		}
		table[i] = entry_t;
	}
...
inline unsigned long long pclmul_lut(unsigned long long crc,  unsigned long long *table)
{
	unsigned i=0, idx;
	unsigned long long clmul = 0LL;
	for(i=0; i<64; i= i+8){
		idx = (crc >> i) &0xff;
		clmul = clmul ^ (table[idx] << i );
	}
	return clmul;	

}

inline int MergeChecksum_lut(int checksum1, int checksum2, unsigned int len_blk2, unsigned long long *table_cl, unsigned long long *table_k ) // LSB: block1|block2 :MSB
{

	int total_checksum;
	unsigned long long k1, crc1_t; 
	k1 = table_k[len_blk2];
	crc1_t = (unsigned long long )(checksum1) & 0xffffffffLL;
	//crc1_t = pclmul(crc1_t, k1);
	gen_table(table_cl, k1);
	crc1_t = pclmul_lut(crc1_t,  table_cl);
	total_checksum = baidu_crc32_qw((char *)(&crc1_t), 0, 8);
	total_checksum = total_checksum ^ checksum2;
	return total_checksum;
}

inline int MergeChecksum(int checksum1, int checksum2, unsigned int len_blk2, unsigned long long *table_cl ) // LSB: block1|block2 :MSB
{

	int total_checksum;
	unsigned long long k1, crc1_t,poly = POL; 
	k1 = mod2(0x1LL, poly, 33, len_blk2*8);
	crc1_t = (unsigned long long )(checksum1) & 0xffffffffLL;
	//crc1_t = pclmul(crc1_t, k1);
	gen_table(table_cl, k1);
	crc1_t = pclmul_lut(crc1_t,  table_cl);
...
inline void gen_k(unsigned long long *table_k, unsigned len) //entry size is len:0,1,2,3...len-1
{
...
inline static unsigned int crc32c_sw( unsigned int crc, const char *data, unsigned int length)
{
        while (length--)
...
inline static unsigned int crc32c_lut(char const * b,unsigned int off,unsigned int len, unsigned int crc) {
