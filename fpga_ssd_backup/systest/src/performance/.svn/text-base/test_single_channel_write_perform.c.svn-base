   ...
   // char device[16];
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < MEASURE; j++) {
           ...
           gen_next_id(id_cur,&next_id,1,44);
		   id_cur=next_id;
        }
...
int main(void)
{
  ...
  //  const  int  SSD_MAX_CHANNEL=44;
    perform_t perform[44];     /* each channel perform data */
    int ssd_flag;    
    BlockId id,next_id;
    enum SSD_TEST_RET_TYPE ret;  
    /*open the ssd device*/
	   
	SSD_CASE_INFO("w-p","01","OPEN THE SSD DEVICE");
	...
	SSD_CASE_INFO("r-p","02","test the write performance");
    gen_first_id(&id);
    for (channel = 0; channel < SSD_MAX_CHANNEL; channel++) {
    ...
    FILE *fp = fopen("log_single_channel_perform_write", "w");
    if (NULL == fp) {
    ...
    for (channel = 0; channel < SSD_MAX_CHANNEL; channel++) {
	 ...
	 SSD_CASE_INFO("w-p","03","close the ssd device");
	SSD_close(0);
