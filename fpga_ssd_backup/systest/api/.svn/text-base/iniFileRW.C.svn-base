...
int	should_fail(const char *section, const char *enable_key,char *re_value,char*re_random,
          int default_value, const char *file)
{
	...
	if(1!=read_profile_int(section, enable_key,default_value,file))
    {
        printf("do not do the fail operation\n");
        return 0;
    }
    else
    {
        value=read_profile_int(section,re_value,1,file);
		printf("%s=%d\n",re_value,value);
		random=read_profile_int(section,re_random,100,file);
        printf("%s=%d\n",re_random,random);
		if(random <= 0){
			return value;
		}
...
//    age = read_profile_int(section,age_key,2,file);
//    printf("%s=%d\n",age_key,age);
