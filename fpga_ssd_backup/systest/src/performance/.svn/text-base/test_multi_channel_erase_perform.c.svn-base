   ...
   // char device[16];
	...
	for(j=1;j<44;j++)
	{
        ...
        for (j = 0; j < SSD_THREAD; j++) {
             if (pthread_create(&tidp[j], NULL, thr_fn_delete, &para[j]) != 0) {
...
int main(void)
{
  ...
  //  int SSD_MAX_CHANNEL=1;
    ...
    /*open the ssd device*/
	SSD_CASE_INFO("d_p","01","OPEN THE SSD DEVICE");
	...
	SSD_CASE_INFO("d_p","02","calculate each channel perform");
    	gen_first_id(&id);
    for (channel = 0; channel < 1; channel++) {
    ...
    FILE *fp = fopen("log_multi_channel_perform_delete", "w");
    if (NULL == fp) {
    ...
    for (channel = 0; channel < 1; channel++) {
        if (0 == channel) {
            fprintf(fp, "the multi channel perform summary delete:\n");
    ...
    SSD_CASE_INFO("d_p","03","SSD close"); 
    SSD_close(0);
