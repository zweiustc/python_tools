...
int id2channel(struct ssd_blockid *id)
{
...
//	printf("channel: %d, blocks: %d\n", channel, blocks);
	...
	if(ERASE == op) {
		...
		for(i = 0; i < blocks; i++) {
            ...
            temp_channel = id2channel(&id);
            char_channel = (char )temp_channel;
            memset(buf, char_channel, BLOCKSIZE);
		...
		//	}
			...
			for(j = 0; j < BLOCKSIZE; j++) {
				if(buf[j] != rd_buf[j]) {
					...
					printf("check error:the data written = %d vs the data read = %d in the %d -th byte of ID %016lX%016lX\n", buf[j], rd_buf[j], j, id.m_nHigh, id.m_nLow);
//					goto out;
				}
			}
#endif
