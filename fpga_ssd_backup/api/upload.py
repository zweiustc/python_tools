...
group.add_option( "--style_check", action = "store_true",
                 dest = "style_check", default = False,
                 help = ( "run code style check on cooder"   ) )
...
def RunShellWithReturnCode( command, print_output = False,
  ...
  """
    #when in linux, get the real path of command svn, through look up 'which svn', alias in '~/.bashrc' or '/etc/bash_profile' 
    if sys.platform.startswith('lin') and\
       len(command)>0  and \
       command[0]=='svn':
        real_svn = ''        
        try:
            handle = subprocess.Popen("which svn", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            output = handle.readline()
            if output and not output.startswith("which:"):
                real_svn = output[0:len(output)-1]
        except:
            pass
        if not real_svn:
            patstr = "^alias\s+svn\s*="
            patobj = re.compile(patstr)            
            try:
                temp_file = open(os.environ['HOME']+"/.bashrc", "r")
                temp_content = temp_file.readlines()
                for line in temp_content:                    
                    if patobj.match(line.strip()):
                        line_parts = line.split('=')
                        line_parts[-1] = line_parts[-1].strip()
                        real_svn = line_parts[-1][1:len(line_parts[-1])-1].strip() # remove ' and whitespace
                        break
            except:
                pass
        if not real_svn:
            try:
                temp_file = open("/etc/bash_profile", "r")
                temp_content = temp_file.readlines()
                for line in temp_content:                    
                    if patobj.match(line.strip()):
                        line_parts = line.split('=')
                        line_parts[-1] = line_parts[-1].strip()
                        real_svn = line_parts[-1][1:len(line_parts[-1])-1].strip()
                        break
            except:
                pass
        if real_svn:
...
def RealMain( argv, data = None ):
  ...
  not response_body.startswith( "Issue updated." ):
    ...
    try:
        ...
        pass
    need_check_syle_result = api_service.need_check_style(issue)
    if options.style_check:
        api_service.check_style_trigger(issue,manual=1)
    elif need_check_syle_result['result']==1:
        api_service.check_style_trigger(issue,manual=0)
    if options.execute:
        cmd = options.execute
        print_data = False
...
class APIService():
    ...
    def latest_revision(self,issue_id):
        ...
        result = self.__json(result_str, success, False)
        return result
    def need_check_style(self,issue_id):
        url="%s/api/needstylecheck/%s?token=redoocnekot"%(self.server,issue_id)
        result_str,success = self.http_service.get(url)
        result = self.__json(result_str, success, False)
        return result
    def check_style_trigger(self,issue_id,manual=0):
        url="%s/api/toolcheck/%s?token=redoocnekot&manual=%d"%(self.server,issue_id,manual)
        self.http_service.get(url)
