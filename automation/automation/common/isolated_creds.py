...
from automation import clients
...
from automation.services.identity.v2.json import identity_client as v2_identity
...
class IsolatedCreds(cred_provider.CredentialProvider):
    ...
    def _create_creds(self, suffix="", admin=False, roles=None):
        ...
        client_set = clients.AdminManager()
        compute_client = client_set.servers_client

        current_existing_az = compute_client.list_availability_zones()
        if not current_existing_az:

            # build an new az for tempest test
            input_az_name = 'az_' + str(uuid.uuid4())
            new_az = compute_client.add_availability_zone(input_az_name)
            az_name = new_az['az_name']

            # bind all hosts to the specified az
            compute_client.bind_all_hosts_to_az(az_name)
        # bind az with tenant
        else:
            az_name = current_existing_az[0]['az_name']
        compute_client.associate_az_tenant(az_name, tenant_id)
        ...
        for creds in six.itervalues(self.isolated_creds):
            ...
            try:
                if CONF.service_available.neutron:
                    ...
                    nets = net_cl.list_networks(name='HA network tenant %s' %
                                                creds.tenant_id)
                    if nets and nets['networks']:
                        net_cl.delete_network(nets['networks'][0]['id'])
