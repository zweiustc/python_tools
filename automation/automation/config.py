...
ComputeGroup = [
    cfg.StrOpt('admin_password',
               default="kingsoft!@#123",
               help="The password of the used image mirror"),
    ...
    cfg.IntOpt('build_timeout',
               ...
               help="Timeout in seconds to wait for an instance to build. "
                    ...
                    "inherit this value."),
    cfg.IntOpt('check_delete_interval',
               default=30,
               help='Time in seconds between instance delete all checks.'),
    cfg.IntOpt('check_delete_timeout',
               default=300,
               help='Timeout in seconds to wait for a instance to become '
                    'deleted.'),
    ...
    cfg.StrOpt('floating_ip_range',
               ...
               help='Unallocated floating IP range, which will be used to '
                    ...
                    'pool.'),
    cfg.StrOpt('zabbix_server',
               default='10.160.57.98',
               help='Zabbix server ip address'),
    cfg.StrOpt('zabbix_user',
               default='admin',
               help='Zabbix server user'),
    cfg.StrOpt('zabbix_password',
               default='zabbix',
               help='Zabbix server password'),
    cfg.StrOpt('security_group',
               default="test_sg",
               help="The security group which having policys to allow full "
                    "access. This is not a required option, but some cases may"
                    " need it to get full firewall access "),
]
...
NetworkGroup = [
    ...
    cfg.StrOpt('tenant_network_name',
               default='test_network',
               help="Tenant network name."),
    cfg.StrOpt('tenant_subnet_name',
               default='test_subnet',
               help="Tenant subnet name."),
    cfg.StrOpt('subnet_cidr',
               default='192.168.2.0/24',
               help="Tenant subnet cidr"),
    cfg.IntOpt('subnet_ip_version',
               default=4,
               help="Tenant subnet ip version."),
    cfg.StrOpt('router_name',
               default='test_router',
               help="Tenant router name."),
    cfg.StrOpt('firewall_name',
               default='test_firewall',
               help="Tenant firewall name."),
    cfg.StrOpt('external_network',
               default='external',
               help="External network name."),
    cfg.StrOpt('sec_group_name',
               default='test_security_group',
               help="External security grouop name."),
...
VolumeGroup = [
    ...
    cfg.IntOpt('check_delete_interval',
               default=30,
               help='Time in seconds between volume delete all checks.'),
    cfg.IntOpt('check_delete_timeout',
               default=300,
               help='Timeout in seconds to wait for a volume to become '
                    'deleted.'),
...
class AutomationConfig(object):
    DEFAULT_CONFIG_DIR = os.path.join(
        ...
        "etc")
    DEFAULT_CONFIG_FILE = "automation.conf"
    ...
    def __init__(self, config_path=None):
        ...
        config_files = []
        failsafe_path = "/etc/automation/" + self.DEFAULT_CONFIG_FILE

        if config_path:
            path = config_path
        else:
            # Environment variables override defaults...
            conf_dir = os.environ.get('AUTOMATION_CONFIG_DIR',
                                      self.DEFAULT_CONFIG_DIR)
            conf_file = os.environ.get('AUTOMATION_CONFIG',
        ...
        if os.path.isfile(path):
            _CONF([], project='automation', default_config_files=config_files)
        else:
            _CONF([], project='automation')
        logging.setup(_CONF, 'automation')

        log_path = _CONF.log_dir
        if not os.path.exists(log_path):
            os.mkdir(log_path)

        LOG = logging.getLogger('automation')
        LOG.info("Using automation config files %s" % path)
        register_opts()
        self._set_attrs()


class AutomationConfigProxy(object):
    _config = None
    _path = None
    ...
    def __init__(self):
        if not self._config:
            ...
            self._config = AutomationConfig(config_path=self._path)
