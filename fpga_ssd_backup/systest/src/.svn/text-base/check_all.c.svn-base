...
int main(void)
{
	...
	SSD_CASE_INFO("","ALL","check the information in SSD ");
	...
	printf("the result is %llx",util_flag);
	for(i=0;i<44;i++){
			util_flag = SSD_util_test(0, i);
			SSD_RESULT("uril","channel ", " ",util_flag,i,0 );
	}
   
    
//   	gb_filecount=0;
	SSD_CASE_INFO("","03","check the ftw info");
   	ssd_flag= SSD_ftw(0,print_all_id, 0);
	if (ssd_flag!= SSD_OK) {
		ret = SSD_FTW_ERROR;
		SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
	...
	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("","ALL","check the information in SSD test pass");
	else
		SSD_CASE_INFO("","ALL","check the information in SSD test pass ");
