...
void each_channel_perform(const struct  BlockId id, perform_t *perform)
{
   // char device[16];
    ...
    for (i = 0; i < 3; i++) {
        ...
        TIMES=SSD_THREAD*MEASURE;
	
	gen_first_id(&id_cur);
        for(j=0;j<TIMES;j++){
		//id_cur=store_write[j];
		gen_next_id(id_cur,&next_id,1,1);
		id_cur=next_id;
		store_write_temp[j]=id_cur;
		store_offset_temp[j]=0;
		store_read_len_temp[j]=(id_cur.m_nHigh & 7) == 0 ? BT_INDEX_SIZE : BT_DATA_SIZE;
	...
	para[j].start_id=store_write[0];
        ...
        for (j = 0; j < SSD_THREAD; j++) {
             if (pthread_create(&tidp[j], NULL, thr_fn_read, &para[j]) != 0) {
            	SSD_CYCLE_INFO("read",i );

                SSD_ERR("","","cant not creat thread delete");
		 }
        }
        for (j = 0; j < SSD_THREAD; j++) {
	...
	printf("the vol is %ld",vol);
	printf ("the s is %f",((e_time.tv_sec - s_time.tv_sec) + (e_time.tv_usec - s_time.tv_usec) / 1000000.0));
	printf("the speed is %f",temp_perform);
...
int main(void)
{
  ...
  //  int SSD_MAX_CHANNEL=1;
    ...
    /*open the ssd device*/
	SSD_CASE_INFO("r_p","01","OPEN THE SSD DEVICE");
	...
	SSD_CASE_INFO("r_p","02","calculate each channel perform");
    gen_first_id(&id);
    for (channel = 0; channel < 1; channel++) {
    ...
    FILE *fp = fopen("log_multi_channel_perform_read", "w");
    if (NULL == fp) {
    ...
    for (channel = 0; channel < 1; channel++) {
        if (0 == channel) {
            fprintf(fp, "the multi channel perform summary read:\n");
  	...
  	SSD_CASE_INFO("r_p","03","SSD close");
	SSD_close(0);
