...
//	printf("channel: %d, blocks: %d\n", channel, blocks);
	...
	if(ERASE == op) {
		...
		for(j = 0; j < BLOCKSIZE; j++)
			...
			//buf[j] = j % 256 ;
#ifdef CRC
		crc_wr = baidu_crc32_fast(buf + CRC_BLOCK_HEADER, 0, BLOCKSIZE - CRC_BLOCK_HEADER, table);
        err_cnt = 0;
#endif
        *(int *)buf = crc_wr;
        *(int *)(buf + 4) = BLOCKSIZE;
...
//				goto out;
			...
			/*check data*/
#ifdef CRC
		    crc_rd = baidu_crc32_fast(rd_buf + CRC_BLOCK_HEADER, 0, BLOCKSIZE - CRC_BLOCK_HEADER, table);
			if(crc_wr != crc_rd) {
				//if(ret % 128)
                check_error = 1;
...
int main(int argc, char *argv[])
{
	...
	ope.op = WRRD;
	ope.blocks = 100; // (test_blocks == 0) ? DATABLOCKS : test_blocks;
	ope.channel = -1;

    printf("start write & read !\n");
	printf("step1: write + read 44 * %u blocks\n", ope.blocks);
	operate_blocks(ope);
	//SSD_close(0);
    ret = SSD_flush(0);
    if(SSD_OK != ret) {
		printf("SSD_flush error %d\n", ret);
		return -1;
	}
    ...
    /*write + read 44 * 8000 blocks*/
	ope.op = ERASE;
	ope.blocks = 20; // (test_blocks == 0) ? DATABLOCKS : test_blocks;
	ope.channel = -1;

    printf("start erase !\n");
	printf("step2: erase 44 * %u blocks\n", ope.blocks);
	operate_blocks(ope);

    /*write + read 44 * 8000 blocks*/
	ope.op = WRRD;
	ope.blocks = 50; // (test_blocks == 0) ? DATABLOCKS : test_blocks;
	ope.channel = -1;

    printf("start write & read !\n");
	printf("step3: write + read 44 * %u blocks\n", ope.blocks);
	operate_blocks(ope);
