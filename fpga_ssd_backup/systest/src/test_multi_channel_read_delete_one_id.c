...
int main(void)
{
	...
	SSD_CASE_INFO("22-02","ALL","TEST the multi thread read delete one id");

        SSD_CASE_INFO("22-02","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("22-02","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("22-02","01","OPEN THE SSD DEVICE");
        }

	SSD_CASE_INFO("22-02","02","gen random one channel id");
    	TIMES=45;
    	gen_order_id(TIMES,1);  //gen the id 44*20
    	gen_randm_id(TIMES);
    

	SSD_CASE_INFO("22-02","03","write times");
    	for (i=0;i<TIMES;i++){
	...
	SSD_CASE_INFO("22-02","04","44 thread read delete one id");
	SSD_THREAD=300;
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_read, &para) != 0) {
			...
			SSD_CYCLE_INFO("read",i );
                        SSD_ERR("22-02","04","cant not creat thread read");
			}
		}else{
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
			...
			SSD_CYCLE_INFO("join",i );
                        SSD_ERR("22-02","04","cant not join thread d");
		}
 
	}
	

	SSD_CASE_INFO("22-02","05","ssd_close");
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("22-02","ALL","TEST the multi thread read delete one id test pass");
	else
		SSD_CASE_INFO("22-02","ALL","TEST the multi thread read delete one id test fail");
