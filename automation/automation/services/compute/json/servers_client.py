...
from automation.api_schema.response.compute.v2_1 import servers as schema
...
class ServersClient(service_client.ServiceClient):
    ...
    def add_availability_zone(self, az_name):
        """add an az to the platform."""
        url = 'os-az-setting/add_availability_zone/action'
        post_body = {'az_name': az_name}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
        return service_client.ResponseBody(resp, body)

    def remove_availability_zone(self, az_name):
        """remove an az from the platform."""
        url = 'os-az-setting/remove_availability_zone/action'
        post_body = {'az_name': az_name}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
    ...
    def add_host_for_availability_zone(self, az_name, host):
        """add host to specified az."""
        url = '/os-az-setting/add_host_for_availability_zone/action'
        post_body = {'az_name': az_name, 'host_name': host}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
        return service_client.ResponseBody(resp, body)

    def remove_host_for_availability_zone(self, az_name, host):
        """remove host from the specified az."""
        url = '/os-az-setting/remove_host_for_availability_zone/action'
        post_body = {'az_name': az_name, 'host_name': host}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
    ...
    def associate_az_tenant(self, az_name, tenant_id):
        """bind tenant with specified az."""
        url = '/os-az-setting/az-tenant-bind/action'
        post_body = {'az_name': az_name, 'tenant_id': tenant_id}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
        return service_client.ResponseBody(resp, body)

    def disassociate_az_tenant(self, az_name, tenant_id):
        """unbind tenant with specified az."""
        url = '/os-az-setting/az-tenant-unbind/action'
        post_body = {'az_name': az_name, 'tenant_id': tenant_id}
        post_body = json.dumps(post_body)
        resp, body = self.post(url, post_body)
        body = json.loads(body)
    ...
    def bind_all_hosts_to_az(self, az_name):
        """bind all hosts belonging to default az to the specified az."""
        hosts_set = self.display_default_nova_az()
        for single_host in hosts_set:
            host_name = single_host['host_name']
            self.add_host_for_availability_zone(az_name, host_name)
    ...
    def get_server_network_qos(self, server_id):
        resp, body = self.get("os-qos-setting/vif-get-detail/%s/action" %
                              server_id)
        body = json.loads(body)
        return service_client.ResponseBody(resp, body)
    ...
    def set_server_network_qos(self, server_id, current_qos_id, new_qos_id):
        params = {
            ...
            "current_qos_id": current_qos_id,
            "new_qos_id": new_qos_id
        }

        req_body = json.dumps(params)
        resp, body = self.post("os-qos-setting/vif-update/action", req_body)
        return service_client.ResponseBody(resp, body)

    def get_network_qos_types(self):
        resp, body = self.get("os-qos-setting/list_vif_instance_qos_types")
        body = json.loads(body)
        return body.get('vif_qos_list')

    def capture_image(self, server_id, name, description=None, **kwargs):
        kwargs['name'] = name
        kwargs['description'] = description
        return self.action(server_id, 'captureBootVolume', None,
                           **kwargs)
