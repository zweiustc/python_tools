...
int main(void)
{
	...
	SSD_CASE_INFO("26-02","ALL","TEST the multi thread  write delete for all channel");

        SSD_CASE_INFO("26-02","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("26-02","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("26-02","01","OPEN THE SSD DEVICE");
        } 
  
    
	SSD_CASE_INFO("26-02","02","gen randon one channel id");
	...
	SSD_CASE_INFO("26-02","03","every thread  to write delelte one id");
   	struct thread_coeff para_1[100];
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_write, &para_1[i]) != 0) {
                        ...
                        SSD_ERR("26-02","03","cant not creat thread write");
			}
		}else{
                if (pthread_create(&tidp[i], NULL, thr_fn_delete, &para_1[i]) != 0) {
                        ret = SSD_TEST_ERROR;
						SSD_CYCLE_INFO("delete",i );
                        SSD_ERR("26-02","03","cant not creat thread delelte");
                        }
		}
	}
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
                        ...
                        SSD_ERR("26-02","03","cant not join thread d");		
}
 
	}
    
     	SSD_CASE_INFO("26-02","04","ssd_close");
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("26-02","ALL","TEST the multi thread  write delete all channel test pass");
	else
		SSD_CASE_INFO("26-02","ALL","TEST the multi thread  write delete all channel  test fail");
