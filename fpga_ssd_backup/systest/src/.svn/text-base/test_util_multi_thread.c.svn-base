...
int main(void)
{
	...
	SSD_CASE_INFO("04-02","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("04-02","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("04-02","01","OPEN THE SSD DEVICE");
        } 

    

	SSD_CASE_INFO("04-02","02","random write TIMES"); 
    	TIMES=400;
	...
	for (i = 0; i < 150; i++) {
		...
		if(i<50){
		  if (pthread_create(&tidp[i], NULL, thr_fn_delete, &para_1[i]) != 0) {
				...
				SSD_ERR("04-02","03","cant not creat thread delete");
		     	ret = SSD_TEST_ERROR;
		  }
		}
		else if(i>=50&&i<100){
		  if (pthread_create(&tidp[i], NULL, thr_fn_write, &para_1[i]) != 0) {
				...
				SSD_ERR("04-02","03","cant not creat thread write");
				ret = SSD_TEST_ERROR;
		   }
		}
		else {
		  if (pthread_create(&tidp[i], NULL, thr_fn_util, NULL) != 0) {
		    	SSD_CYCLE_INFO("delete",i );
                        
				SSD_ERR("04-02","03","cant not creat thread util");
	...
	for (i = 0; i < 150; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
				...
				SSD_ERR("04-02","03","cant not join thread d");   
				ret = SSD_TEST_ERROR;
		}
      	}
	...
	SSD_CASE_INFO("04-02","04","SSD DEVICE CLOSE");
        SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("04-02","ALL","multi thread write delete util test pass");
	else
		SSD_CASE_INFO("04-02","ALL","multi thread write delete util test fail");
	return 0;
}
