...
int main(void)
{
	...
	SSD_CASE_INFO("13-02","ALL","TEST the multi thread read each for 44 channel");

        SSD_CASE_INFO("13-02","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("13-02","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("13-02","01","OPEN THE SSD DEVICE");
        }
 
	SSD_CASE_INFO("13-02","02","gen random all channel id");
	NUM=45;
	SSD_THREAD=100;
	...
	SSD_CASE_INFO("13-02","03","random TIMES write");
    	for (i=0;i<TIMES;i++){
    		id=store_write[i];
	...
	SSD_CASE_INFO("13-02","04","each thread for all channel");	
	...
	for (i = 0; i < SSD_THREAD; i++) {
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_read, &para[i]) != 0) {
			...
			SSD_CYCLE_INFO("read",i );
                        SSD_ERR("13-02","04","cant not creat thread read");
			}
	}
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
			...
			SSD_CYCLE_INFO("join",i );
                        SSD_ERR("13-02","04","cant not join thread d");
		}
 
	}
	
    
   	SSD_CASE_INFO("13-02","05","ssd_close"); 
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("13-02","ALL","TEST the multi thread read each for 44 channel test pass");
		
	else
		SSD_CASE_INFO("13-02","ALL","TEST the multi thread read each for 44 channel test fail");
