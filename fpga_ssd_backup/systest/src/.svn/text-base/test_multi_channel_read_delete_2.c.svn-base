...
int main(void)
{
	...
	SSD_CASE_INFO("24-02","ALL","TEST the multi thread read delete for all channel");

        SSD_CASE_INFO("24-02","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("24-02","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("24-02","01","OPEN THE SSD DEVICE");
        } 
  
    
	SSD_CASE_INFO("24-02","02","gen randon one channel id");
	...
	SSD_CASE_INFO("24-02","03","every thread  to read delete all channel");
   	struct thread_coeff para_1[100];
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_read, &para_1[i]) != 0) {
                        ...
                        SSD_ERR("24-02","03","cant not creat thread read");
			}
		}else{
                if (pthread_create(&tidp[i], NULL, thr_fn_delete, &para_1[i]) != 0) {
                        ret = SSD_TEST_ERROR;
						SSD_CYCLE_INFO("delete",i );
                        SSD_ERR("24-02","03","cant not creat thread delelte");
                        }
		}
	}
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
                        ...
                        SSD_ERR("24-02","03","cant not join thread d");		
}
 
	}
    
     	SSD_CASE_INFO("24-02","04","ssd_close");
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("24-02","ALL","TEST the multi thread  read delete all channel test pass");
	else
		SSD_CASE_INFO("24-02","ALL","TEST the multi thread  read delete all channel  test fail");
