...
int main(void)
{
	...
	SSD_THREAD=440;
	NUM=80;//each thead 100id,divide to 10 team in the thread
	id.m_nLow  =0;
	id.m_nHigh =0x07;
	
	for(j=0;j<20;j++){

    	struct thread_coeff para[440];//440 thread ,10 thread for one channel
    	para[0].start_id=id;
    	para[0].step=440;
    	para[0].start_num=0;
...
*/
	id.m_nLow  =para[439].start_id.m_nLow + 440*NUM + 1;
        id.m_nHigh =0x07;
