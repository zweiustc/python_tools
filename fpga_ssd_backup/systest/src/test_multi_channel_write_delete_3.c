...
int main(void)
{
	...
	SSD_CASE_INFO("26-03","ALL","TEST the multi thread  write delete for one channel");

        SSD_CASE_INFO("26-03","01","OPEN THE SSD DEVICE");
        if ((ssd_flag = SSD_open(0)) != SSD_OK) {
        SSD_RESULT("excepted","actual", "else",SSD_OK,ssd_flag,0 );
        SSD_ERR("26-03","01","OPEN THE SSD DEVICE");
        ret = SSD_OPEN_ERROR;
        } else {
        SSD_SUCCESS("26-03","01","OPEN THE SSD DEVICE");
        } 
  
    
	SSD_CASE_INFO("26-03","02","gen randon one channel id");
    	NUM=4;
	SSD_THREAD=300;
	...
	SSD_CASE_INFO("26-02","03","every thread  to write delelte one id");
   	struct thread_coeff para_1[300];
		...
		if (pthread_create(&tidp[i], NULL, thr_fn_write, &para_1[i]) != 0) {
                        ...
                        SSD_ERR("26-03","03","cant not creat thread write");
			}
		}else{
                if (pthread_create(&tidp[i], NULL, thr_fn_delete, &para_1[i]) != 0) {
                        ret = SSD_TEST_ERROR;
						SSD_CYCLE_INFO("delete",i );
                        SSD_ERR("26-03","03","cant not creat thread delelte");
                        }
		}
	}
	...
	for (i = 0; i < SSD_THREAD; i++) {
		if (pthread_join(tidp[i], &tret) != 0) { 
                        ...
                        SSD_ERR("26-03","03","cant not join thread d");		
}
 
	}
    
     	SSD_CASE_INFO("26-03","04","ssd_close");
	SSD_close(0);

	if (SSD_TEST_OK == ret)
		SSD_CASE_INFO("26-03","ALL","TEST the multi thread  write delete one channel test pass");
	else
		SSD_CASE_INFO("26-03","ALL","TEST the multi thread  write delete one channel  test fail");
