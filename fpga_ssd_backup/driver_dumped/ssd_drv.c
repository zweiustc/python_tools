        ...
        if (likely(s6_pending > 0)) {
            ...
            for (j = 0; (j < SSD_CHANNELS_IN_S6) && s6_pending; j++) {
                ...
                if (s6_pending < 0) {
                     ...
                     * hardware error used for write failed retry currently only
                     */
                    ch_ctl->errors <<= 7;
...
static int ssd_remap_block(struct ssd_device *sdev, ssd_u8 channel)
{
    ...
    map = kmalloc(SSD_TABLE_SIZE, GFP_KERNEL);
    if (!map) {
        ssd_log(SSD_LOG_ERR, "kmalloc map failed (line %d)\n", __LINE__);
        ret = -SSD_ERR_NOMEM;
        goto out;
    }
...
static int ssd_id_write(struct ssd_device *sdev, struct ssd_write_param *wparam)
{
    ...
    if (BLOCKSIZE[IBLOCK] == wparam->len)
        ...
        ssd_log(SSD_LOG_ERR, "ssd_id_write: SSD_ERROR_PARAM_WRLEN,len = %u (line %d)\n", wparam->len, __LINE__);
    ...
    if (-SSD_ID_NO_EXIST != ret) {
        ssd_log(SSD_LOG_ERR, "ssd_id_write: get_id_info failed, ret[%d], id.High=%llx id.Low=%llx channel=%u block=%u len=%u (line %d)\n",
                            ret, wparam->id.m_nHigh, wparam->id.m_nLow, channel, id_info.block, id_info.len, __LINE__);
    ...
    if (-SSD_ERR_ALLOC_BLOCK == index || index >= SSD_MAX_VIRBLOCKS_PER_CHANNEL) {
        ssd_log(SSD_LOG_ERR, "ssd_id_write: alloc logic block failed, index[%d] (line %d)\n", index, __LINE__);
    ...
    cmd = cmd_value(index * SSD_PAGES_PER_BLOCK, wparam->len * SSD_PAGES_PER_BLOCK, SSD_FLAG_WRITE);
    ...
    if (copy_from_user(sdev->kbuftable_cpu[channel], wparam->buf, wparam->len * SSD_BLOCK_SIZE)) {
    ...
    if (ret != SSD_OK) {
        ssd_log(SSD_LOG_WARNING, "ssd_id_write: channel op failed, ret[%d], id.High=%llx id.Low=%llx channel=%u block=%u len=%u (line %d)\n",
                            ret, wparam->id.m_nHigh, wparam->id.m_nLow, id_info.channel, id_info.block, id_info.len, __LINE__);
        /*write retry*/
        if(0 != (block_ret_flag = ret >> 7)) {
            for (offset = 0; offset < BLOCKSIZE[type]; fail_flag <<= 1, offset++) {
                if (0 == (block_ret_flag & fail_flag))
                    continue;

                for (retry_times = 0; retry_times < SSD_MAX_WRITE_RETRY; retry_times++) {
                    cmd = cmd_value((index + offset) * SSD_PAGES_PER_BLOCK, SSD_PAGES_PER_BLOCK, SSD_FLAG_WRITE);	
                ...
                if (retry_times == SSD_MAX_WRITE_RETRY) {
                    ssd_log(SSD_LOG_ERR, "ssd_id_write: write retry failed, ret=[%d], id.High=%llx, id.Low=%llx, channel=%u, block=%u, offset=%u (line %d)\n",
                            ret, wparam->id.m_nHigh, wparam->id.m_nLow, channel, id_info.block, offset, __LINE__); 
                    goto fail;
                }
            }
        }
    ...
    if (ret < 0) {
        ssd_log(SSD_LOG_ERR, "ssd_id_write: set_id_info failed, ret[%d], id.High=%llx id.Low=%llx channel=%u block=%u len=%u (line %d)\n",
                            ret, wparam->id.m_nHigh, wparam->id.m_nLow, id_info.channel, id_info.block, id_info.len, __LINE__);
        ret = -SSD_ERR_NOMEM;
        goto fail;
    }
    ...
    part_stat_add(cpu, &(sdev->gdisk[0]->part0), sectors[1], (wparam->len) * 2*1024*2);      
...
static int ssd_read_flush(struct ssd_device *sdev, struct ssd_meta_version *version)
{
    ...
    struct semaphore *sema;
    struct ssd_metadata *metadata;
    int channel = version->vbp.block_page.block / RESERVE_BLOCKS_PER_CHANNEL;
    
    if (g_opened_flag == SSD_OPENED)
        return -SSD_ERR_NOT_OPENED;

    if (version->vbp.block_page.block >= RESERVE_BLOCKS_META)
    ...
    //read metadata
    ret = ssd_read_metadata(sdev, version->vbp.block_page.block, version->vbp.block_page.page);
    if (SSD_OK != ret) {
    ...
    if (SSD_OK != meta_val)
        return meta_val;
    //update sdev info 
    sdev->meta_pos.block_pos = version->vbp.block_page.block;
    sdev->meta = *metadata;
    sdev->meta_pos.page = version->vbp.block_page.page;
    sdev->meta.metadata_magic = FLUSH_MAGIC_CLOSE;

    //read table data based on metadata and buffer it in kbuf_flush
    buff = sdev->kbuf_flush;
    memset(buff, 0, sdev->meta.block_num * SSD_BLOCK_SIZE);
...
static inline int ssd_mark_erase_count(struct ssd_device *sdev, struct ssd_mark_erase_count_param *mecparam)
{
    ...
    */
    ...
    map = kmalloc(count * sizeof(struct erase_entry_map), GFP_KERNEL);
    if (!map) {
        ssd_log(SSD_LOG_ERR, "memory alloc fail of ssd_device (line %d)\n", __LINE__);
        ret = -ENOMEM;
        goto fail;
